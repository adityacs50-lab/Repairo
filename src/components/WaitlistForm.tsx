...
export function WaitlistForm() {
  ...
  const handleSubmit = async (e: React.FormEvent, isGoogle: boolean = false) => {
    e.preventDefault();
    const targetEmail = isGoogle && !email ? "google-user@heyrepairo.in" : email;
            ...
            {!googlePrompt ? (
              ...
              >
                <input
                  ...
                  className="w-full h-11 bg-surface-elevated border border-emerald-500/50 rounded-full px-5 py-2.5 text-ink text-sm transition-all focus:outline-none font-sans"
                />
                <button
                  type="submit"
                  disabled={status === "loading"}
                  className="h-11 bg-emerald-500 text-black font-semibold text-sm px-6 py-2.5 rounded-full hover:bg-emerald-400 transition-all shrink-0 cursor-pointer w-full sm:w-auto"
                >
                  {status === "loading" ? "Registering..." : "Confirm Google Beta"}
                </button>
              </motion.form>
            )}
